		...
		if (widget != null && resize) {
			...
			if (cleanWidget) {
				...
				if (GXT.isTouch()) {
					...
					// widgets wanting 100% must handle scrolling themselves to avoid extraneous
					// scroll bars
				...
				ScheduledCommand adjustLayout = new ScheduledCommand() {
					@Override
					public void execute() {
						// after the layout has finished, fetch the adjusted widget size (including any
						// possible overflow & scrollbars)
						...
						// if the actual size differs from the calculated size, position the widget
						// again because it's not right
				...
				// once the widget is dirty, ensure it can't ever be clean again even if the
				// user restores original bounds
